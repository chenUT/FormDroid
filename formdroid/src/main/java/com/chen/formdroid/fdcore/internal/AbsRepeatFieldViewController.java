...
import com.chen.formdroid.fdcore.template.fields.AbsRepeatField;
 ...
 */
public abstract class AbsRepeatFieldViewController<T extends AbsRepeatField> extends AbsInputFieldViewController<T> {
    ...
    public AbsRepeatFieldViewController(T field, Fragment frag) {
        super(field, frag);
    }

    /**
     * override this to tell the dialog whether it will create a clearToDefault button
     * @return true if the dialog need a clearToDefault button to clearToDefault all input fields
     */
    ...
    public List<AbsInputFieldViewController> getDialogViewControllers(Fragment fragment, int resultIndex){
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            AbsInputFieldViewController ctrl = field.getViewControllerInternal(fragment);
            if(ctrl != null) {
                result.add(ctrl);
            }
        }
